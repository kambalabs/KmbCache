...
class CacheManagerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function canRefreshExpiredCache()
    {
        ...
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor(CacheManager::KEY_NODE_STATISTICS), serialize($lastMonth));
        $this->cacheStorage->setItem(CacheManager::KEY_NODE_STATISTICS, serialize(['nodesCount' => 1]));
        ...
        $this->assertEquals(serialize($expectedNodeStatistics), $this->cacheStorage->getItem(CacheManager::KEY_NODE_STATISTICS));
        ...
        $this->assertEquals(serialize($this->now), $this->cacheStorage->getItem(CacheManager::refreshedAtKeyFor(CacheManager::KEY_NODE_STATISTICS)));
    }
    ...
    public function cannotRefreshExpiredCacheWhenRefreshIsPending()
    {
        ...
        $this->assertEquals(serialize(['nodesCount' => 1]), $this->cacheStorage->getItem(CacheManager::KEY_NODE_STATISTICS));
    }

    /** @test */
    public function cannotRefreshExpiredCacheWhenNoCacheIsExpired()
    {
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor(CacheManager::KEY_AVAILABLE_PUPPET_MODULES), serialize($this->now));
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor(CacheManager::KEY_NODE_STATISTICS), serialize($this->now));
        $this->cacheStorage->setItem(CacheManager::KEY_NODE_STATISTICS, serialize(['nodesCount' => 1]));
    ...
    public function canRefreshExpiredWithEnvironment()
    {
        ...
        $this->cacheStorage->setItem($nodesKey, serialize(['nodesCount' => 1]));
        $this->cacheStorage->setItem($modulesKey, serialize(['ntp' => new PuppetModule('ntp', '1.1.2')]));
        $expectedNodeStatistics = ['nodesCount' => 2];
        $this->nodeStatisticsService->expects($this->any())
            ->method('getAllAsArray')
            ->will($this->returnValue($expectedNodeStatistics));
        ...
        $this->assertEquals(serialize($expectedNodeStatistics), $this->cacheStorage->getItem($nodesKey));
        $this->assertEquals(CacheManager::COMPLETED, $this->cacheStorage->getItem(CacheManager::statusKeyFor($nodesKey)));
        $this->assertEquals(serialize($this->now), $this->cacheStorage->getItem(CacheManager::refreshedAtKeyFor($nodesKey)));
        $this->assertEquals(serialize($expectedModules), $this->cacheStorage->getItem($modulesKey));
        $this->assertEquals(CacheManager::COMPLETED, $this->cacheStorage->getItem(CacheManager::statusKeyFor($modulesKey)));
        $this->assertEquals(serialize($this->now), $this->cacheStorage->getItem(CacheManager::refreshedAtKeyFor($modulesKey)));
    }
    ...
    public function canClearCache()
    {
        ...
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor($nodesKey), serialize($this->now));
        $this->cacheStorage->setItem($modulesKey, serialize(['ntp' => new PuppetModule('ntp', '1.1.2')]));
        $this->cacheStorage->setItem(CacheManager::statusKeyFor($modulesKey), CacheManager::COMPLETED);
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor($modulesKey), serialize($this->now));
    ...
    public function canGetNodeStatistics()
    {
        ...
        $this->cacheStorage->setItem($key, serialize($expectedNodeStatistics));
        $this->cacheStorage->setItem(CacheManager::statusKeyFor($key), CacheManager::COMPLETED);
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor($key), serialize($this->now));
    ...
    public function canGetInstallableModules()
    {
        ...
        $this->cacheStorage->setItem($key, serialize($expectedModules));
        $this->cacheStorage->setItem(CacheManager::statusKeyFor($key), CacheManager::COMPLETED);
        $this->cacheStorage->setItem(CacheManager::refreshedAtKeyFor($key), serialize($this->now));
